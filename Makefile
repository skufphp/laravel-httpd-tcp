...
# Сервисы
PHP_CONTAINER=laravel-php-httpd-tcp
HTTPD_CONTAINER=laravel-httpd-tcp
POSTGRES_CONTAINER=laravel-postgres-httpd-tcp
PGADMIN_CONTAINER=laravel-pgadmin-httpd-tcp
...
laravel-install: up ## Создать новый проект Laravel в ./src (full cycle)
	...
	@$(MAKE) setup
	@echo "$(YELLOW)Запуск базовых сервисов...$(NC)"
	docker compose up -d $(PHP_CONTAINER) $(POSTGRES_CONTAINER)
	@echo "$(YELLOW)Установка Laravel через Composer...$(NC)"
	docker compose exec $(PHP_CONTAINER) composer create-project laravel/laravel .
	@echo "$(YELLOW)Генерация ключа и миграции...$(NC)"
	...
	@echo "$(YELLOW)Запуск веб-сервера Apache...$(NC)"
	docker compose up -d $(HTTPD_CONTAINER)
	@echo "$(GREEN)✓ Laravel успешно установлен и запущен!$(NC)"
	@echo "$(GREEN)URL: http://localhost$(NC)"
